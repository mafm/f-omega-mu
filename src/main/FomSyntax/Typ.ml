...
and quantifier atomize symbol (typ : t) =
  ...
  | _ -> [symbol; pp false typ] |> concat |> nest 1 |> parens_if atomize
...
and pp atomize (typ : t) =
  match typ with
  | `Mu (_, `Lam (_, id, kind, body)) -> binding atomize mu_lower id kind body
  | `Mu (_, typ) -> [mu_lower; pp false typ |> parens] |> concat |> nest 1
